...
// CRT enrolment is no longer collected — field retired per management. Kept here
// (commented) so the whole feature can be restored in one place if needed.
// export const CRT_FEE_VALUES = ['Yes', 'No'];

// All three years still render in the form. Only 2030 (1st Year / Bamboo Coder) is
// open; 2028 & 2029 stay visible but disabled (see CLOSED_PASSOUT_YEARS below).
export const PASSOUT_YEARS = ['2030', '2029', '2028'];

// Passout years whose registrations are CLOSED. The form shows these as disabled
// cards with a "Closed" tag, and clicking one surfaces a "registrations closed"
// notice; the API also rejects any submission for them. The 2028 and 2029 batches
// have completed their programs, so both are closed — only 2030 is selectable.
export const CLOSED_PASSOUT_YEARS = ['2029', '2028'];
...
export const FIELD_LIMITS = {
    ...
    collegeEmail: 120,
    mobileNumber: 10,
    // Optional entrance-exam ranks (1st year / 2030 intake). EAPCET ranks run to a
    // few lakh (≤6 digits); JEE Mains CRL can reach ~7 digits — allow a little head-room.
    eapcetRank: 7,
    jeeRank: 8
};
...
export function validateRegistration(raw = {}) {
    const values = {
        ...
        // paidCrtFee: asString(raw.paidCrtFee).trim(), // CRT enrolment retired
        passoutYear: asString(raw.passoutYear).trim(),
        // Optional entrance ranks — digits only, empty string when not provided.
        eapcetRank: asString(raw.eapcetRank).replace(/[^\d]/g, ''),
        jeeRank: asString(raw.jeeRank).replace(/[^\d]/g, '')
    };
    ...
    // CRT enrolment field retired — validation kept commented for easy restore.
    // if (!values.paidCrtFee) {
    //     errors.paidCrtFee = 'Please let us know if you have enrolled to CRT';
    // } else if (!CRT_FEE_VALUES.includes(values.paidCrtFee)) {
    //     errors.paidCrtFee = 'Please select a valid option';
    // }

    // Entrance ranks are OPTIONAL (management students leave them blank). Only
    // validate the format when a value is actually entered.
    if (values.eapcetRank) {
        if (values.eapcetRank.length > FIELD_LIMITS.eapcetRank) {
            errors.eapcetRank = 'Please enter a valid EAPCET rank';
        } else if (!/^\d+$/.test(values.eapcetRank) || Number(values.eapcetRank) === 0) {
            errors.eapcetRank = 'Enter a valid rank (numbers only)';
        }
    }

    if (values.jeeRank) {
        if (values.jeeRank.length > FIELD_LIMITS.jeeRank) {
            errors.jeeRank = 'Please enter a valid JEE Mains rank';
        } else if (!/^\d+$/.test(values.jeeRank) || Number(values.jeeRank) === 0) {
            errors.jeeRank = 'Enter a valid rank (numbers only)';
        }
    }
    ...
    } else if (CLOSED_PASSOUT_YEARS.includes(values.passoutYear)) {
        errors.passoutYear = 'Registrations for this batch are now closed';
    }
