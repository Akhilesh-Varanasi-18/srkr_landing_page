...
import { TORIIMINDS_URL, SRKR_COLLEGE_URL } from '../../lib/site-config';
...
const SrkrHeader = ({ onOpenRegister, onOpenAnnouncements, onOpenWhatsApp, announcementCount = 0 }) => {
            ...
            <header className="srkr-header" style={isScrolled ? { boxShadow: 'var(--srkr-shadow-hover)' } : {}}>
                <div className="srkr-header-container">
                    ...
                    <div className="srkr-header-actions">
                        {announcementCount > 0 && (
                            <div className="srkr-notify-wrap">
                                <button
                                    type="button"
                                    className="srkr-btn-notify"
                                    onClick={() => onOpenAnnouncements && onOpenAnnouncements()}
                                    aria-label={`View ${announcementCount} ${announcementCount === 1 ? 'announcement' : 'announcements'}`}
                                    title="Announcements"
                                >
                                    ...
                                    <span className="srkr-notify-ping" aria-hidden="true" />
                                </button>

                                {/* Eye-catching bouncing flag so the announcement never goes unnoticed. */}
                                <button
                                    type="button"
                                    className="srkr-notify-flag"
                                    onClick={() => onOpenAnnouncements && onOpenAnnouncements()}
                                    aria-hidden="true"
                                    tabIndex={-1}
                                >
                                    <span className="srkr-notify-flag-dot" />
                                    <span>New Announcement{announcementCount === 1 ? '' : 's'}</span>
                                </button>
                            </div>
                        )}
                        <button
